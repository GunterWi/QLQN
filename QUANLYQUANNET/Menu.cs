...
namespace QUANLYQUANNET
{
    public partial class Menu : Form
    {
        ...
        void loadaddmoney()
        {
            ...
            // cmd.CommandText = "select FORMAT(THOIGIANNAPTIEN, 'yyyy-MM-dd HH:mm:ss')as [Thời gian] ,nv.TEN as [Người thu],nt.SOTIEN as [Số tiền] from NAPTIEN nt join NHANVIEN nv on nt.MANV=nv.MANV where THOIGIANNAPTIEN >= DATEADD(day, -2, GETDATE()) AND THOIGIANNAPTIEN <= GETDATE();   ";
        ...
        private void Menu_Load(object sender, EventArgs e)
        {
            ...
            /*Properties.Settings.Default.MaHD = 6; 
        ...
        {
            cmd = con.CreateCommand();
            cmd.CommandText = "select Tenhang, lh.Tenloaihang,SOLUONGTON AS [Số lượng còn],DVT, Dongia from MENU mn join LOAIHANG lh on mn.MALOAIHANG = lh.MALOAIHANG where LH.MALOAIHANG='ML01'";
            adapter.SelectCommand = cmd;
            table = new DataTable();
            adapter.Fill(table);
        ...
        {
            cmd = con.CreateCommand();
            cmd.CommandText = "select Tenhang, lh.Tenloaihang,SOLUONGTON AS [Số lượng còn],DVT, Dongia from MENU mn join LOAIHANG lh on mn.MALOAIHANG = lh.MALOAIHANG where LH.MALOAIHANG='ML02'";
            adapter.SelectCommand = cmd;
            table = new DataTable();
            adapter.Fill(table);
            dvgDichVu1.DataSource = table;
...
/*            DateTime now = DateTime.Now;
        ...
        private void timerphut_Tick(object sender, EventArgs e) // chạy thời gian sử dụng
        {
            for (int i = 0; i < dgvmaytram.Rows.Count - 1; i++)
            {
                if (dgvmaytram.Rows[i].Cells[1].Value.ToString() == "DANG SU DUNG")
                {
                    int phut = (int)dgvmaytram.Rows[i].Cells[4].Value;
                    phut++;
                    dgvmaytram.Rows[i].Cells[4].Value = phut;
                    conlai = conlai - 1;
...
/*                // Tăng thời gian hiện tại lên 1 phút.
        ...
        {
            cmd = con.CreateCommand();
            cmd.CommandText = "UPDATE CTHOADON SET TRANGTHAITT = 'Da thanh toan' FROM CTHOADON ct jOIN HOADON hd ON hd.MAHD = ct.MAHD join MAYKHACH mk on mk.MAMT = hd.MAMT where TENMAYTINH = 'MÁY 1'";
            cmd.ExecuteNonQuery();
            loaddivuvuct();
        }
